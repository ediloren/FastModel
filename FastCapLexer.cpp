...
// CFastCapLexer, a FastCap lexer class - Enrico Di Lorenzo 2001/08/04
//
...
int CFastCapLexer::recognizeTok(unsigned int *position)
{
	...
	do {
		...
		if( c=='0' ) {
			...
			if(ctmp == 'x' || ctmp == 'X') {
				...
				do {
					ctmp = readChar(position);
					m_sString[i] = (char)tolower(ctmp); 
					i++;
				} while (ctmp != '\n' && ishexchar(ctmp) && i < LEXER_MAX_TOK_LEN);
...
bool CFastCapLexer::ishexchar(char c)
{
	if( (c >= '0' && c <= '9') ||
		(c >= 'a' && c <= 'f') ||
		(c >= 'A' && c <= 'F') ) {
